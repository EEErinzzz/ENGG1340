...
//input is the sight that the player have. Player can only see the maze within a circle whose radius is the sight and the center is the player position
{
	for (int i = 1; i < size; i++)
	{
		...
		{
			//print the boundary of the maze
			if ((maze[i][j] != 1 && j == size - 2) || (maze[i][j] != 1 && i == 1) || (maze[i][j] != 1 && i == size - 2) || (maze[i][j] == 0 && j == 1))
			{
				addch(' ' | A_REVERSE);


			}
			else if (abs(i - player_y) * abs(i - player_y) + abs(j - player_x) * abs(j - player_x) <= sight * sight && maze[i][j] == 0)
			{
				addch(' ' | A_REVERSE);
			}
			else if (maze[i][j] == 'X') { printw("X"); refresh(); continue; }
			else
			{
				printw(" "); refresh();
			}
